...
describe(Data) # 62 pps - 107 variables - 62 lines of data 
...
Data_long<-melt(Data_wide, id.var=c("pp", "orde","X0ty", "X1ty", "X2ty", "X3ty", "X4ty", "X5ty", "X6ty", "X7ty", "f1_cc", "f2_cc", "f3_cc", "f4_cc", "f5_cc", "f6_cc", "f7_cc", "f8_cc"), variable_name="variable")
head(Data_long)
...
## compute Contingency Awareness variable: 1 = correct , 2 = incorrect, 3=don't know, 4 = both actions equal
...
Data_long$order<-factor(Data_long$orde) ## order condition: first EPT or first ratings
...
## Model 3: Model 2 + random slope of type for pp

fit3<-lmer(value~type+(1+type|pp)+(1|face), data=Data_long, contrasts = list(type="contr.sum"))
summary(fit3)

anova(fit2,fit3) # no: no random slope --> Model 2 is best

## Model 4: Model 2 + random slope of type for face

fit4<-lmer(value~type+(1|pp)+(1+type|face), data=Data_long, contrasts = list(type="contr.sum"))
summary(fit4)

anova(fit2,fit4) # No better fit 

##### 4. Perform Lmer Analyses Overall ####

## Test effects of fixed factors Type and Order Condition

fit<-lmer(value~type*order+(1|pp)+(1|face), data=Data_long, contrasts = list(type="contr.sum", order="contr.sum"))
summary(fit)
Anova(fit, type='III')

effect('type', fit)       # p=.007
effect('type:order', fit) # p=.45:    no signficant interaction

## Explore main effect of Face Type

Data_long_a<-Data_long[Data_long$type==1,] # approached
mean(Data_long_a$value)  # M = 3.87
sd(Data_long_a$value)

Data_long_av<-Data_long[Data_long$type==2,] #avoided
mean(Data_long_av$value) # M= 3.61
sd(Data_long_av$value)

fit5<-lmer(value~type*order+(1|pp)+(1|face), data=Data_long, contrasts = list(type="contr.treatment", order="contr.sum"))
fm1W <- confint(fit5) # [0.08, 0.46]  confidence intervals

##### 5. Perform Lmer Analyses with contingency Awareness Factor ####

## Test effects of fixed factors Type, Order Condition and Contingency Awareness

Data_long$cc<-ifelse(Data_long$cc==3|Data_long$cc==4, 3, Data_long$cc) # 3 levels of cont awareness: correct, incorrect, indiscriminate
Data_long$cc<-factor(Data_long$cc)

fit6<-lmer(value~type*order*cc+(1|pp)+(1|face), data=Data_long, contrasts = list(type="contr.sum", order="contr.sum",cc="contr.sum" ))
summary(fit6)
Anova(fit6, type='III')

effect('type', fit6)    # p=.23: no significant effect face type 
...
fit6.1<-lmer(value~type*order+(1|pp)+(1|face), data=Data_long.1, contrasts = list(type="contr.sum", order="contr.sum" ))
summary(fit6.1)                                                            
...
mean(Data_long.1_a$value) #4.28
sd(Data_long.1_a$value)
Data_long.1_av<-Data_long.1[Data_long.1$type==2,] #avoided
mean(Data_long.1_av$value) #3.44
sd(Data_long.1_av$value)
fit6.1<-lmer(value~type*order+(1|pp)+(1|face), data=Data_long.1, contrasts = list(type="contr.treatment", order="contr.sum" ))
fm1W <- confint(fit6.1, method="Wald")# [0.47, 1.07]

Data_long.2<-Data_long[Data_long$cc==2, ] # contingency incorrect faces: 
fit6.2<-lmer(value~type*order+(1|pp)+(1|face), data=Data_long.2, contrasts = list(type="contr.sum", order="contr.sum" ))
summary(fit6.2)                                                            
Anova(fit6.2, type='III')
effect('type', fit6.2) # face type effect is not significant .104
plot(effect('type', fit6.2))
Data_long.2_a<-Data_long.2[Data_long.2$type==1,] #approached
mean(Data_long.2_a$value) #3.54
sd(Data_long.2_a$value)
Data_long.2_av<-Data_long.2[Data_long.2$type==2,] #avoided
mean(Data_long.2_av$value) #3.93
sd(Data_long.2_av$value)
fit6.2<-lmer(value~type*order+(1|pp)+(1|face), data=Data_long.2, contrasts = list(type="contr.treatment", order="contr.sum" ))
fm1W <- confint(fit6.2)# [-0.87, 0.08]

Data_long.3<-Data_long[Data_long$cc==3, ] # contingency indiscriminate faces:
fit6.3<-lmer(value~type*order+(1|pp)+(1|face), data=Data_long.3, contrasts = list(type="contr.sum", order="contr.sum" ))
summary(fit6.3)                                                            
Anova(fit6.3, type='III')
effect('type', fit6.3) # face type effect is not significant .20
plot(effect('type', fit6.3))
Data_long.3_a<-Data_long.3[Data_long.3$type==1,] #approached
mean(Data_long.3_a$value) #3.45
sd(Data_long.3_a$value)
Data_long.3_av<-Data_long.3[Data_long.3$type==2,]#avoided
mean(Data_long.3_av$value) #3.70
sd(Data_long.3_av$value)

##### 3. write away final data file #######

write.table(Data_long, "DataRatingsExperiment2.txt", sep="\t") 
