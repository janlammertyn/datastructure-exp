...
describe(Data) # 64 pps - 83 variables - 64 lines of data (1/pp)

##### 2. exclude pp 30 and 64 for making too many errors in EPT ####

Data<-Data[!Data$pp==30, ]
Data<-Data[!Data$pp==64, ]

##### 3. compute explicit rating scores for each face  ####

## build explicit rating score for each face by averageing liking+friendliness ratings
...
## calculate contingency awareness variable per face:
# 1 = contingency correct , 2 = incorrect, 3 =don't know, 4 = both actions equal 
...
Data$f1_cc<-ifelse(Data$X0ac==4, 4, Data$f1_cc)
Data$f2_cc<-ifelse(Data$X1ac==4, 4, Data$f2_cc)
Data$f3_cc<-ifelse(Data$X2ac==4, 4, Data$f3_cc)
Data$f4_cc<-ifelse(Data$X3ac==4, 4, Data$f4_cc)
Data$f5_cc<-ifelse(Data$X4ac==4, 4, Data$f5_cc)
Data$f6_cc<-ifelse(Data$X5ac==4, 4, Data$f6_cc)
Data$f7_cc<-ifelse(Data$X6ac==4, 4, Data$f7_cc)
Data$f8_cc<-ifelse(Data$X7ac==4, 4, Data$f8_cc)

## calculate contingency correct variable (cc)
# total nr of faces correct
...
## calculate contingency incorrect variable (ci)
# total nr of faces incorrect 
...
## calculate contingency indiscriminate variable 1 (dbt)
# total nr of faces participants indicated 'don't know' 
...
## calculate contingency indiscriminate variable 2 (dbt2)
# total nr of faces participants indicated 'both actions equal' 

Data$dbt2<-0
Data$dbt2<-ifelse(Data$f1_cc==4, Data$dbt2+1, Data$dbt2)
Data$dbt2<-ifelse(Data$f2_cc==4, Data$dbt2+1, Data$dbt2)
Data$dbt2<-ifelse(Data$f3_cc==4, Data$dbt2+1, Data$dbt2)
Data$dbt2<-ifelse(Data$f4_cc==4, Data$dbt2+1, Data$dbt2)
Data$dbt2<-ifelse(Data$f5_cc==4, Data$dbt2+1, Data$dbt2)
Data$dbt2<-ifelse(Data$f6_cc==4, Data$dbt2+1, Data$dbt2)
Data$dbt2<-ifelse(Data$f7_cc==4, Data$dbt2+1, Data$dbt2)
Data$dbt2<-ifelse(Data$f8_cc==4, Data$dbt2+1, Data$dbt2)
Data$dbt2_p<-(Data$dbt2/8)*100
