...
void randomise(int ppnr,char mv)	// function for randomizing all variables
{
	...
	//1. ca: contingency instructions or no contingency instructions: yes if ppnr is odd: 1,3,5,7,9,11,...
	if (ppnr%2==1)
		ca=1;
	else
		ca=0;
	...
	// approach-avoidance task: randomise size of face and nr of face (which determines type: appr/avoid)
	...
	// determine blocknumber based on trial number
	for (i = 0; i < NTAATRIALS; i++) {
		aadata[i].block=1 + i/32;
	}

	// evalutive priming task task: randomise nr of face used as prime and type of target 
	...
	for (i = 0; i < NTPRIMETRIALS; i++) {
		...
		if (((primedata[i].targettype==1)&&(primedata[i].hand==1))||((primedata[i].targettype==0)&&(primedata[i].hand==2)))
			...
			primedata[i].xr=6;	// right respose: 'i'
	...
	// determine blocknumber based on trialnumber
	for (i = 0; i < NTPRIMETRIALS; i++) {
