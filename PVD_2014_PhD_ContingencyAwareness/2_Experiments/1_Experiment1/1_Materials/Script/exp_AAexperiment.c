...
void aainstructions2(ppnr)
{
	...
	// half pps do not learn which faces they approach and which they avoid (ca bs-variable=0)	
	if (ca==0){						
	...
	// Half pps do learn which faces they approach and which they avoid (ca bs-variable=1)
	else {							
		ts_printf_centre(0, ay(.95), "Dit zijn de gezichten die u zal moeten benaderen:");
		for (k=0;k<NTFACES;k++){
			if (face[k].nr==0)
				ts_blit(bmp[k+33], ax(-.75), ay(.50));
		}
		...
		ts_printf_centre(0, ay(.15), "Dit zijn de gezichten die u zal moeten vermijden:");
		for (k=0;k<NTFACES;k++){
			if (face[k].nr==4)
				ts_blit(bmp[k+33], ax(-.75), ay(-.25));
		}
		...
		ts_printf_centre(0, -ay(.70), "Onthoud goed welke gezichten u moet benaderen of vermijden. U zal hierover vragen moeten beantwoorden na de taak.");
	}
...
void primeinstructions(ppnr)			
{
	...
	ts_font(REG,20,ARIAL);
	ts_printf(ax(-.80), ay(.80), "Je hebt het eerste deel van dit experiment afgerond !");
	...
	// Half pps use left key 'e' for pos words  and 'i' for neg words(hand ept bs-variable=1)
 	if ((ppnr%4==0)|(ppnr%4==1)){		
	...
	// Half pps use left key 'e' for neg words  and 'i' for pos words (hand ept bs-variable=1)
	else {
...
void debriefinginstructions()		
{
	...
	for(l = 0; l<NTFACES; l++){
		...
		ts_blit(bmp[l+1], 0, ay(.30));
	    	ts_printf_centre(0, ay(-.3), " 1 = benaderen      2 = vermijden    3 = beide acties even vaak");
		ts_printf_centre(0, ay(-.36), "  Toets cijfer 1, 2 of 3 in : ");
		do {
			ts_scanf(ax(.3), ay(-.36), "%d", &face[l].action);}
		while (face[l].action<1||face[l].action>3);
		if (face[l].nr<4)
			face[l].type=1;
		else face[l].type=2;
		if (face[l].action==face[l].type)
			...
			face[l].ca=0;
		ts_printf_centre(0, ay(-.5), "Hoe zeker ben je van dit antwoord?");	// confidence variable for each face: how certain of contingency?
		...
		ts_printf_centre(0, ay(-.74), "In hoeveel procent van de keren dat je dit gezicht zag moest je die actie uitvoeren?"); // pcy variable for each face: how certain in percentage
	...
	ts_printf_centre(0, ay(.70), "Hoe positief vind je het woord 'benader'?");
	ts_printf_centre(0, ay(.60), "0       1        2       3       4       5        6       7");
	ts_line (ax(-.6),ay(.56),ax(.6),ay(.56));
	ts_printf_centre(0,ay(.53),"helemaal niet         niet                     wel             heel veel");
	...
	ts_printf_centre(0, ay(.10), "Hoe positief vind je het woord 'vermijd'?");
	ts_printf_centre(0, ay(.0), "0       1        2       3       4       5        6       7");
	ts_line (ax(-.6),ay(-.04),ax(.6),ay(-.04));
	ts_printf_centre(0,ay(-.07),"helemaal niet         niet                     wel             heel veel");
...
int main()
{    
	//ts_priority(HIGH);  		// set priority to high: change this when collecting data ! 
	ts_scrsize(SIZE2);
	...
	for (i = 0; i < NTAATRIALS; i++) {         
        	aatrial(i);
		if (i == 96)
			blockaa();	// pauze screen after 1/2 of trials
	}
	...
	for (i = 0; i < NTPRIMETRIALS; i++) {
        primetrial(i);  
		if (i==NTPRIMETRIALS/4)
			blocka();	
		if (i==NTPRIMETRIALS/4*2)
			blockb();
		if (i==NTPRIMETRIALS/4*3)
			blockc();  
	}
	primewrite(nr);			// write away data of evaluative priming task

	explicitinstructions();		// 5. explicit ratings
	debriefinginstructions();	// 6. answer debriefing questions
	explicitwrite(nr);		// write away data of rating and debriefing questions
		
	// 7. thank pps for participation
