/* 
	Script for study PVD_2014_ContAwareness
	Author: Pieter Van Dessel
	Production date: 01/10/2013
*/
...
struct {
char word[16];
} posword[NTPRIMETARGETPOS]; 	// creates 6 posword variables

struct {
char word[16];
} negword[NTPRIMETARGETNEG]; 	// creates 6 negword variables

int ca; 		// variable for seeing if participant was in contingency aware condition: received instructions or not
...
int asoften;		// variable that will encode whether participant thought all faces were as often approached as avoided
...
void initialise()	// function for initialising all variables
{
	...
	if (fp == NULL)
		ts_fatal("Error while reading posprime.stim");
	...
	if (fp2 == NULL)
		ts_fatal("Error while reading negprime.stim");
...
void aawrite(int ppnr) // function for writing away approach-avoidance trial data
{
	...
	fp = fopen(fname, "a+"); 
	if (fp == NULL)
		ts_fatal("Error while writing file %s \n", fname);
	int i;
...
void explicitwrite(int ppnr) // function for writing away data of explicit questions
{
	...
	fprintf(fp, "%d\t", ca);			// condition of participant: contingency instructions yes or no
	...
	fprintf(fp, "%d\t", face[7].nr);		// eight face ...
